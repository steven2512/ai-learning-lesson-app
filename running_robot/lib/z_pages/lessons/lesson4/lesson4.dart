// lib/z_pages/lessons/lesson4/lesson4.dart
// ✅ LessonFour — single-step lesson that hosts the Falling Words game

import 'package:flame/components.dart' show Vector2;
import 'package:flutter/material.dart';
import 'package:running_robot/core/app_router.dart';
import 'package:running_robot/z_pages/assets/lessonAssets/continueButton.dart';
import 'package:running_robot/z_pages/assets/lessonAssets/icon_button.dart';
import 'package:running_robot/z_pages/assets/lessonAssets/progress_bar.dart'
    as flutter_ui_bar;
import 'package:running_robot/game/decorations/progress_bar.dart'
    show LessonProgressBar;
import 'package:running_robot/game/events/event_type.dart'
    show EventProgressBar;

// ⬇️ Your game step moved here (Step7 → Step0 for this lesson)
import 'package:running_robot/z_pages/lessons/lesson4/lesson4_1.dart';

class LessonFour extends StatefulWidget {
  final AppNavigate onNavigate;
  const LessonFour({super.key, required this.onNavigate});

  @override
  State<LessonFour> createState() => _LessonFourState();
}

class _LessonFourState extends State<LessonFour> {
  int currentStep = 0;

  // Only one step — pick whatever vertical offset you like for the step area
  final Map<int, double> topOffsets = const {
    0: 50, // matches where you liked Step 7 to sit
  };

  int get totalStages => 1;
  bool _lessonCompleted = false;
  bool _stepAnswered = false; // gate the Continue button

  late IconButtonWidget<void> returnButton;

  @override
  void initState() {
    super.initState();
    returnButton = IconButtonWidget<void>(
      iconPath: 'assets/images/x_icon.png',
      tint: Colors.black87,
      size: 22,
      onPressed: (_) => widget.onNavigate(const RouteMainMenu(tab: 0)),
    );
  }

  @override
  Widget build(BuildContext context) {
    final double topOffset = topOffsets[currentStep] ?? 120;

    return Scaffold(
      backgroundColor: Colors.white,
      body: Stack(
        children: [
          // ▼ Game first (painted below)
          Positioned.fill(
            top: topOffset,
            bottom: 100,
            child: _buildCurrentStep(),
          ),

          // ▲ Progress bar on top — absorb taps so basket doesn't move when tapping it
          Positioned(
            top: 70,
            left: MediaQuery.of(context).size.width / 2 - (279 / 2),
            child: AbsorbPointer(
              // ★ added
              absorbing: true,
              child: flutter_ui_bar.LessonProgressBar(
                totalStages: totalStages,
                currentStage: _lessonCompleted ? totalStages : currentStep,
              ),
            ),
          ),

          // ▲ Close button on top — will receive taps before the game's Listener
          Positioned(top: 69, left: 30, child: returnButton),

          // Continue button (unchanged)
          Positioned(
            bottom: 40,
            left: 0,
            right: 0,
            child: Center(
              child: _lessonCompleted
                  ? const SizedBox.shrink()
                  : _buildContinueButton(),
            ),
          ),
        ],
      ),
    );
  }

  Widget _buildContinueButton() {
    // Only step → require game completion
    if (currentStep == 0 && !_stepAnswered) {
      return const SizedBox.shrink();
    }

    return ContinueButton(
      onPressed: () {
        // Only 1 step → go straight to end-of-lesson
        setState(() => _lessonCompleted = true);

        final endBar = LessonProgressBar(
          position: Vector2.zero(),
          stages: totalStages,
        );
        for (int i = 0; i < totalStages; i++) {
          endBar.switchPhase(EventProgressBar.proceed);
        }

        widget.onNavigate(
          RouteEndLesson(
            xp: 50,
            streak: 1,
            progressBar: endBar,
            chapterProgress: 4, // 🔧 adjust if needed
            totalChapterLessons: 10, // 🔧 adjust if needed
            topText: "Lesson 4 complete! 🎉",
            repeatLesson: const RouteLesson(4),
            nextLesson: const RouteMainMenu(), // or RouteLesson(5) later
            illustrationPath: null,
          ),
        );
      },
    );
  }

  Widget _buildCurrentStep() {
    switch (currentStep) {
      case 0:
        // The game (formerly LessonStepSeven) now exposed as LessonStepZero
        return LessonStepZero(
          onStepCompleted: () => setState(() => _stepAnswered = true),
        );
    }
    return const SizedBox.shrink();
  }
}
