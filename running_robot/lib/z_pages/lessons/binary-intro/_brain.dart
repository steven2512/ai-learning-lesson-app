...
import 'package:running_robot/z_pages/lessons/binary-intro/comp_0101.dart';
import 'package:running_robot/z_pages/lessons/binary-intro/binary_intro.dart';
import 'package:running_robot/z_pages/lessons/binary-intro/bin_example.dart';
import 'package:running_robot/z_pages/lessons/binary-intro/unicode.dart';
import 'package:running_robot/z_pages/lessons/binary-intro/binary_game.dart';

import 'photo.dart'; // StepZero
import 'music.dart'; // StepOne (quiz step)
import 'comp_0101.dart' show ComputerSeeZeroOne;

class BinaryIntroBrain extends StatefulWidget {
  final AppNavigate onNavigate;
  const BinaryIntroBrain({super.key, required this.onNavigate});

  @override
  State<BinaryIntroBrain> createState() => _BinaryIntroBrainState();
}

class _BinaryIntroBrainState extends State<BinaryIntroBrain> {
  int currentStep = 0;
  ...
  Widget _buildCurrentStep() {
    if (currentStep == 0) return const HumanSeePhoto();
    if (currentStep == 1) return const HumanHearMusic();
    if (currentStep == 2) {
      return ComputerSeeZeroOne(
        onStarted: () {
    ...
    if (currentStep == 4) return BinaryExample();
    if (currentStep == 5) return HelloInUnicode();
    if (currentStep == 6) {
      return BinaryDragDropGame(
        onCompleted: () {
